...
int has_loop(int *arr, int n, int looplen, int *ls, int *le)
{
		...
		int *secondPart = malloc (sizeof(int)*(looplen));	//DAYI KODU KOPYALAMA PATLARIZ BAK FLKD NDSOGNJSOIDGJSPD
...
void check_loop_iterative(void (*f)(int, int, int, int *), int xs, int seqlen, int *loop, int *looplen)
{
	...
	int *ary = malloc(sizeof(int)*seqlen);		//DAYI KODU KOPYALAMA PATLARIZ BAK FLKD NDSOGNJSOIDGJSPD
	f(xs,0,seqlen,ary);
	...
	if (*looplen >= 2)   //Loop len can not be shorter than 3 elementh
	{
...
{
	if (digit > seqlen - 1)		//DAYI KODU KOPYALAMA PATLARIZ BAK FLKD NDSOGNJSOIDGJSPD
		return ;
	...
	else
	{
		...
		int firstDigit = findFirstDigit(ptr[digit]);	//find first digit of every item in array by ascending order
		h[firstDigit-1] += 1;							//Add one every histogram of number that you find
...
int main()
{
	...
	int *histogram = (int *)calloc(9,sizeof(int));	//Histogram of first digits
